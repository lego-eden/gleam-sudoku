...
pub fn set(sudoku: Sudoku, row: Int, col: Int, num: Int) -> Sudoku {
  let target_cell: Cell =
    result.lazy_unwrap(dict.get(sudoku.cells, #(row, col)), fn() { panic })
  let new_grid =
    dict.insert(sudoku.cells, #(row, col), cell.into(target_cell, num))
  let new_grid = {
...
pub fn to_string(sudoku: Sudoku) -> String {
  let grid: List(List(String)) = {
    ...
    let current_cell =
      dict.get(sudoku.cells, #(row, col)) |> result.lazy_unwrap(fn() { panic })
